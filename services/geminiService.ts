...
const HUMAN_DIRECTIVE = "DIRECTIVA HUMANA OBLIGATORIA: Para que el texto parezca escrito por un profesor de universidad, tendrás que usar un lenguaje sofisticado y culto, pero sin que resulte arcaico o inaccesible. Evita el uso de palabras rebuscadas del diccionario o un lenguaje demasiado antiguo (por ejemplo, evita términos como 'aditamiento', 'advenimiento' o 'pergueñar'). Mantén un tono SERIO y un estilo de ESCRITURA LITERARIO, pero claro y comprensible para una audiencia profesional moderna. Agrega emociones sutiles, expresiones humanas y lenguaje conversacional. Usa frases con ritmos variados. Evita un tono demasiado perfecto o robótico. Añade conectores suaves y naturales: “ademas”, “la verdad es que”, “y es que”, “por lo tanto”, entre otros. Cuando parafrasees ideas de fuentes teóricas, DEBES incluir una cita en el texto inmediatamente después del parafraseo, siguiendo el formato de autor-fecha de la norma APA 7ma edición (por ejemplo, (Apellido, Año)). Asegúrate de incluir 2 o 3 de estas citas a lo largo de la publicación. NO incluyas una lista de 'Referencias' al final.";

const buildTextPrompt = (data: FormData): string => {
  const wordCountRange = data.wordCount.match(/\d+-\d+/)?.[0] || '150-200';

  let prompt = `Eres un experto en marketing de redes sociales y también experto en educación con sólidos conocimientos en tecnología educativa, evaluación y curriculum, especializado en crear contenido atractivo y profesional para LinkedIn. Tu tarea es escribir una publicación para LinkedIn basada en las siguientes especificaciones. La publicación debe estar en español.

${HUMAN_DIRECTIVE}

**Tema:** ${data.theme}
**Rango de palabras:** Aproximadamente ${wordCountRange} palabras.
`;
...
Asegúrate de que la publicación final no exceda los 3000 caracteres.
...
export const generateContent = async (formData: FormData) => {
  try {
    ...
    let textResponse;
    const wordCountRange = formData.wordCount.match(/\d+-\d+/)?.[0] || '150-200';

    if (formData.generativeLinks.length > 0) {
      const groundingPrompt = `Eres un experto en marketing de redes sociales y también experto en educación con sólidos conocimientos en tecnología educativa, evaluación y curriculum, especializado en crear contenido atractivo y profesional para LinkedIn. Tu tarea es escribir una publicación para LinkedIn basada en las siguientes especificaciones. La publicación debe estar en español.

Basado en el tema "${formData.theme}" y la información encontrada en la web, escribe una publicación profesional para LinkedIn en español con un rango de palabras aproximado de ${wordCountRange}. ${formData.details ? `Considera estos detalles: ${formData.details}` : ''}

${HUMAN_DIRECTIVE}

La publicación debe ser profesional, informativa y fomente la interacción. Formateala con espaciado adecuado, emojis relevantes y hashtags pertinentes al final.
Asegúrate de que la publicación final no exceda los 3000 caracteres.
